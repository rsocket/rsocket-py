...
from examples.tutorial.step5_1.shared import FileSender, FileReceiver
...
class ChatClient:
    ...
    async def upload(self, file_name, content):
        self._rsocket.request_channel(
            Payload(content, composite(
                route('file.upload'),
                metadata_item(ensure_bytes(file_name), chat_filename_mimetype)
            )),
            FileSender(file_name)
        )

    async def download(self, file_name):
        self._rsocket.request_stream(
            Payload(
                metadata=composite(route('file.download'),
                                   metadata_item(ensure_bytes(file_name), chat_filename_mimetype)))
        ).subscribe(FileReceiver())
