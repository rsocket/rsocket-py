...
class TestFeatureLease:
    ...
    async def test_request_response_with_server_side_lease_works(self, lazy_pipe):
    ...
    async def test_request_response_with_client_and_server_side_lease_works(self, lazy_pipe):
        class Handler(BaseRequestHandler):
            async def request_response(self, request: Payload):
                future = asyncio.Future()
                future.set_result(Payload(b'data: ' + request.data,
    ...
    async def test_request_response_with_lease_client_side_exception_requests_late(self, lazy_pipe):
        class Handler(BaseRequestHandler):
            async def request_response(self, request: Payload):
                future = asyncio.Future()
                future.set_result(Payload(b'data: ' + request.data,
    ...
    async def test_server_rejects_all_requests_if_lease_not_supported(self, lazy_pipe):
        async with lazy_pipe(client_arguments={'honor_lease': True}) as (server, client):
            with pytest.raises(asyncio.exceptions.TimeoutError):
                await asyncio.wait_for(client.request_response(Payload(b'invalid request')), 3)

    @pytest.mark.skip(reason='TODO')
    async def test_request_response_with_lease_server_side_exception(self, lazy_pipe):
        class Handler(BaseRequestHandler):
            async def request_response(self, request: Payload):
                future = asyncio.Future()
                future.set_result(Payload(b'data: ' + request.data,
